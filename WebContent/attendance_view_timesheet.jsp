...
<body>
	...
	<div class= "main_frame">
		...
		<%
			...
			thisMonthCalendar.add(Calendar.MONTH, 1);
			DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("HH時mm分");
		...
		%>
			...
			%>
						...
						<%= workTime.getStartTime().format(dateFormat) %><% } %></td>
						<td><% if(workTime.getFinishTime() != null) { %>
						<%= workTime.getFinishTime().format(dateFormat) %><% } %></td>
						<td><% if(workTime.getBreakStartTime() != null) { %>
						<%= workTime.getBreakStartTime().format(dateFormat) %><% } %></td>
						<td><% if(workTime.getBreakFinishTime() != null) { %>
						<%= workTime.getBreakFinishTime().format(dateFormat) %><% } %></td>
