...
public class WorkTimeDAO {
	...
	public List<WorkTime> selectWorkTimeThisMonthList(String employeeCode,String thisMonth)
		...
		while(rs.next()){
			...
			workTime.setWorkDate(LocalDate.parse(rs.getString(1),
					DateTimeFormatter.ofPattern("[]H:mm:ss")) );
			DateTimeFormatter dtf = DateTimeFormatter.ofPattern("[]H:mm:ss");
			if(rs.getString(2) != null) {
				LocalTime startTime = LocalTime.parse(rs.getString(2), dtf);
				workTime.setStartTime(startTime);
			}
			if(rs.getString(3) != null) {
				LocalTime finishTime = LocalTime.parse(rs.getString(3), dtf);
				workTime.setFinishTime(finishTime);
			}
			if(rs.getString(4) != null) {
				LocalTime breakStartTime = LocalTime.parse(rs.getString(4), dtf);
				workTime.setBreakStartTime(breakStartTime);
			}
			if(rs.getString(5) != null) {
				LocalTime breakFinishTime = LocalTime.parse(rs.getString(5), dtf);
